...
Pour faire ce TP, vous devez créer un nouveau projet et utiliser un répertoire GitHub dès le début.
...
:::note <Highlight color="tip"><span style={{color: 'white'}}>1 point</span></Highlight> 📂 Fichiers dans le projet et structure du fichier analyse.py
    Votre projet doit contenir exactement les 2 scripts Python mentionnés ci-dessous, un fichier `README.md` qui explique la structure du projet et comment utiliser les deux scripts, et un fichier `.gitignorea` et rien d'autre (ne laissez traîner aucun script inutile).
    ```
    tp2/
    ├── README.md        # Contient la description du projet et les instructions pour exécuter les scripts
    ...
    └── rapport/
        ├── PNG # ce dossier contiendra les figures générées par votre script analyse.py
        └── sommaire.txt # ce fichier sera généré par votre script analyse.py

    ```
    Téléchargez le fichier <a href={useBaseUrl('/ressources/tp2/autocorrection.py')} download="autocorrection.py">`autocorrection.py`</a> et le fichier <a href={useBaseUrl('/ressources/tp2/.gitignore')} download=".gitignore" type="text/plain">`.gitignore`</a> et placez-les dans votre projet. \
...
- Les commits décrivent l'avancement du projet dans un français sans faute [(voir instructions)](https://simonll.github.io/420-SN1.git)
...
- **#4** - Quelles sont les espèces d'arbres (en français) que l'on retrouve dans le `Parc Aimée-Miville`? Trier les alphabétiquement (Il est normal que les mots accentués apparaissent à la fin).\
...
- N’utilisez aucune boucle (`for`, `while`) ni aucune liste de compréhension;
...
:::note <Highlight color="tip"><span style={{color: 'white'}}>1 point</span></Highlight> ✏️ Création du fichier sommaire.txt
...
- **Contenu** : Il doit inclure à la fois :
    - l'énoncé de la question (ex : « Quel est... ? ») et la réponse obtenue (ex : « 52 »).
    - Les réponses ne doivent contenir ni accolades `{}`, ni crochets `[]`, ni parenthèses `()`, ni guillemets.
...
<TabItem value="P1F1" label="Figure 1" default>
        ...
        **ASTUCES**
        ...
        - Pour chacun des types d'arbres de la liste :
            ...
            - Ajoutez-les au graphique à l'aide d'un nuage de points avec une taille de marqueur de `0.1`;
    ...
    <TabItem value="F3" label="Figure 3">
        ...
        **ASTUCES**
        ...
        - La légende doit avoir :
            ...
            - les étiquettes de la légende doivent avoir une taille de `3.5`;
...
:::tip Autocorrection des 6 questions d'analyse
...
<iframe
    ...
    src="https://www.youtube.com/embed/1T-q7Y9dGkk?si=7wnS3KA64kD0bfcq"
