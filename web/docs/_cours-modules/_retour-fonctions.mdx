...
:::info `return` — retourner une ou plusieurs valeurs

Les valeurs retournées servent de sortie à la fonction ou *output*.

Une fonction peut retourner plusieurs valeurs, séparées par des virgules. Les valeurs retournées peuvent être **déballées** (unpacked) directement lors de l'appel :
...
:::danger
Pour cette raison, **ne capturez jamais le résultat d'une fonction qui ne retourne rien dans une variable**, et **ne l'affichez jamais avec `print()`**.<br />
