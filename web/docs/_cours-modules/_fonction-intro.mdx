...
```
Dans cet exemple, les lignes `y = 2 * x` et `return y` font partie du corps de la fonction `double`, tandis que la ligne `z = 3` est en dehors de la fonction.
...
Une fonction retourne une valeur qui n'est pas affichée automatiquement : il faut la stocker dans une variable ou l'afficher avec `print()` pour la voir.
