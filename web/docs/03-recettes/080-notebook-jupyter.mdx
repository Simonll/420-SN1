...
## Accéder à un notebook Colab

Colaboratory (Colab) est un environnement de notebook Jupyter gratuit qui fonctionne entièrement dans le cloud. Il est particulièrement utile pour le développement en Python, notamment pour l'apprentissage automatique et l'analyse de données, car il offre un accès gratuit à des GPU (Graphiques Processing Units ou unité de calcul graphique) et TPU (Tensor Processing Units ou unité de traitement de tensoriel).

### Créer un nouveau notebook Colab

1.  Ouvrez votre navigateur web et allez à [Google Colab](https://colab.research.google.com/).
2.  Assurez-vous d'être connecté à votre compte Google.
3.  Cliquez sur `Fichier > Nouveau carnet de notes` (File > New notebook).
4.  Un nouveau notebook vide sera créé et ouvert dans votre navigateur. Vous pouvez commencer à y écrire du code ou du texte.

### Ouvrir un fichier .ipynb existant dans Colab

1.  Ouvrez votre navigateur web et allez à [Google Colab](https://colab.research.google.com/).
2.  Cliquez sur `Fichier > Importer un carnet de notes` (File > Upload notebook).
3.  Vous aurez plusieurs options pour importer :
    *   **Importer** : Téléchargez un fichier `.ipynb` depuis votre ordinateur.
    *   **Google Drive** : Ouvrez un notebook directement depuis votre Google Drive.
    *   **GitHub** : Chargez un notebook à partir d'un dépôt GitHub (vous devrez fournir l'URL).
4.  Sélectionnez l'option désirée et suivez les instructions pour ouvrir votre notebook.
